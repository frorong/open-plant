export interface StackBlitzProjectDefinition {
  title: string;
  description: string;
  template: "node";
...
const DEMO_DATA = `import { buildClassPalette, closeRing, createCircle, createRectangle } from 'open-plant';
...
function createProject(appSource: string, title: string, description: string): StackBlitzProjectDefinition {
  return {
    title,
    description,
    template: "node",
    files: {
      ...
      "src/demoData.js": DEMO_DATA,
      "src/App.jsx": appSource,
    },
  };
}

const COMPOSITION_APP = `import React, { useState } from 'react';
...
const POINTS_APP = `import React, { useRef, useState } from 'react';
...
const DRAW_APP = `import React, { useState } from 'react';
...
const HEATMAP_APP = `import React, { useState } from 'react';
          ...
          <WsiViewer source={demo.source} style={{ width: '100%', height: '100%' }}>
            <HeatmapLayer
              ...
              onStats={(next) => setStats([
                'input points -> ' + demo.heatmapData.count,
...
const LOW_LEVEL_APP = `import React, { useEffect, useRef, useState } from 'react';
...
const SOURCE_APP = `import React, { useMemo, useState } from 'react';
...
const CLIP_APP = `import React, { useEffect, useMemo, useState } from 'react';
import {
  PointLayer,
  RegionLayer,
  WsiViewer,
...
const HIT_INDEX_APP = `import React, { useEffect, useRef, useState } from 'react';
...
const GEOMETRY_APP = `import React, { useState } from 'react';
...
const UTILS_APP = `import React, { useState } from 'react';
...
export function getStackBlitzProject(demo: string): StackBlitzProjectDefinition | null {
  switch (demo) {
    case "composition":
